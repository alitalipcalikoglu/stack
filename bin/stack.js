...
export class Cli {
  ...
  static async main(argv) {
    const { command, flags } = Cli.parse(argv);
    ...
    switch (command) {
      ...
      default:
        console.log('usage: atc-stack setup [--root dir] [--host host] [--public] [--install] [--admin-email e] [--admin-password p] | up | down | status | dev');
        return 2;
    }
  }
}
